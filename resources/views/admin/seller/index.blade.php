...
@section('content')
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-8">
        ...
        <div class="mb-4 sm:mb-0">
            <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">All Sellers</h1>
        </div>
        <!-- Right: Actions -->
        <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
            ...
            <!-- Add customer button -->
            <a class="btn bg-indigo-500 hover:bg-indigo-600 text-white" href="{{ route('admin.seller.add') }}">
                <svg class="w-4 h-4 fill-current opacity-50 shrink-0" viewBox="0 0 16 16">
                    <path
                        d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
                </svg>
                <span class="hidden xs:block ml-2">Add Seller</span>
            </a>
    ...
    <div class="bg-white shadow-lg rounded-sm border border-slate-200 mb-8">
        <header class="px-5 py-4">
            <h2 class="font-semibold text-slate-800">All Sellers <span
                ...
                <table class="table-auto w-full divide-y divide-slate-200 customdatatable">
                    ...
                    <thead class="text-xs uppercase text-slate-500 bg-slate-50 border-t border-slate-200">
                        <tr>
                            ...
                            <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-semibold text-left">Seller Name</div>
                            </th>
                            <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-semibold text-left">Seller Email</div>
                            </th>
                            <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-semibold text-left">Seller Phone</div>
                            </th>
                    ...
                    <tbody class="text-sm" x-data="{ open: false }">
                        ...
                        @forelse ($users as $user)
                            {{-- @php
                           $file= $user->media ? asset('storage/images/profile/'.$user->media()->first()->file) : asset('assets/images/dummy-user.png');
                           dd($file);
                        @endphp --}}
                            <tr>
                                ...
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    @switch($user->is_blocked)
                                        @case(1)
                                            <a href="javascript:void(0)" data-block="0" data-uuid="{{ $user->uuid }}"
                                                data-table="users" data-message="Unblock"
                                                class="inline-flex font-medium bg-slate-100 text-slate-500 rounded-full text-center px-2.5 py-0.5 changeUserBlock">Blocked</a>
                                        @break

                                        @case(0)
                                            <a href="javascript:void(0)" data-block="1" data-table="users" data-message="block"
                                                data-uuid="{{ $user->uuid }}"
                                                class="inline-flex font-medium bg-emerald-100 text-emerald-600 rounded-full text-center px-2.5 py-0.5 changeUserBlock">Unblocked</a>
                                        @break
                                    @endswitch
                                </td>
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap ">
                                    <div class="flex items-center text-center">
                                        <div class="m-1.5">
                                            ...
                                            <a class="btn btn-sm border-slate-200 hover:border-slate-300 text-slate-600"
                                                href="{{ route('admin.seller.edit', $user->uuid) }}">
