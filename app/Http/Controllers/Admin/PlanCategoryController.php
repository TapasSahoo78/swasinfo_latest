...
class PlanCategoryController extends BaseController
{
        ...
        if ($request->post()) {
            ...
            try {
                ...
                if ($isCategoryCreated) {
                    DB::commit();
                    return $this->responseRedirect('admin.subscription.category.list', 'Category created successfully', 'success', false);
                }
            } catch (\Exception $e) {
                DB::rollback();
    ...
    public function editCategory(Request $request, $uuid)
    {
        ...
        $listCategories = $this->categoryService->listCategories($filterConditions, 'id', 'asc'); */
        $categoryId = uuidtoid($uuid, 'plan_categories');
        $categoryData = $this->categoryService->findCategoryById($categoryId);

        if ($request->post()) {
            $request->validate([
                'plan_categories' => 'required'
            ]);
            DB::beginTransaction();
            /* try { */
            //$iscategoryUpdated = $this->categoryService->createOrUpdateCategory($request->except('_token'), $categoryId);
            $iscategoryUpdated = PlanCategory::where('id', $categoryId)->update([
                'category_name' => $request->category_name
            ]);
                if ($iscategoryUpdated) {
                    DB::commit();
                    return $this->responseRedirect('admin.subscription.category.list', 'Category updated successfully', 'success', false);
                }
            /* } catch (\Exception $e) {
                DB::rollback();
                logger($e->getMessage() . ' -- ' . $e->getLine() . ' -- ' . $e->getFile());
        ...
        return view('admin.category.edit-category', compact('categoryData'));
    }
